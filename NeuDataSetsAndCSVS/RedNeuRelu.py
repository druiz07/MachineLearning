...
def cost(theta_list, X, Y, lambda_):
    ...
    reg_term = 0
    # Término de regularización para evitar el sobreajuste (L2)
...
def backprop(theta_list, X, y, lambda_):
    ...
    for i in range(len(theta_list) - 1, 0, -1):
        delta = np.dot(deltas[0], theta_list[i][:, 1:]) * relu_gradient(zs[i - 1])
        deltas.insert(0, delta)

    # Gradients
    grads = [np.dot(d.T, a) / m for d, a in zip(deltas, activations[:-1])]

    # Término de regularización L2 en los gradientes (excluye términos de sesgo)
