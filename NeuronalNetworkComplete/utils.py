...
def initialize_parameters(layer_sizes):
    ...
    return theta_list
def checkNNGradients(costNN, alpha,reg_param=0):
    """
    ...
    XnoBias = debugInitializeWeights(input_layer_size - 1, m)
    X = np.hstack([np.ones((m, 1)), XnoBias])
    ...
    # Definir el número de neuronas por capa (incluyendo la capa de entrada y salida)
    layer_sizes = [X.shape[1], 3, 5] 

    # Inicializar parámetros de la red neuronal
    theta_list = initialize_parameters(layer_sizes)

    # Compute Gradient
    cost, grads = costNN(theta_list,X, ys, reg_param)
    ...
    def reduced_cost_func(p):
        ...
        return  costNN(theta_list,X, ys, reg_param,alpha)[0]
